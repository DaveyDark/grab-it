...
const Product = ({ id }: { id: string }) => {
  return (
    <div className='flex max-w-full p-20 border min-h-screen mx-20 my-20 flex-row'>
      <div className='mr-48'>
        <Image src={productDetails[0].img} alt='product' width={800} height={800} />
      </div>
      <div className='flex flex-col w-full pr-10 '>
        <p className='text-xl text-[#60454E]'>Home/Chips/<span className='text-[#7F7A7A]'>{productDetails[0].name}</span></p>
        ...
        <div className='flex flex-row justify-between '>
          <div className='flex gap-3'>
            ...
            <p className='text-sm text-[#10622A]'>{productDetails[0].discountedPrice}</p>
          </div>
